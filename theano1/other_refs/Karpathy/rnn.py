...
# data I/O
data = open('/Users/morteza/Downloads/input.txt', 'r').read() # should be simple plain text file
chars = list(set(data))
data_size, vocab_size = len(data), len(chars)
print 'data has %d characters, %d unique.' % (data_size, vocab_size)
char_to_ix = { ch:i for i,ch in enumerate(chars) }  # create mps
...
def lossFun(inputs, targets, hprev):
  ...
  for t in xrange(len(inputs)):
    ...
    xs[t][inputs[t]] = 1
    hs[t] = np.tanh(np.dot(Wxh, xs[t]) + np.dot(Whh, hs[t-1]) + bh) # hidden state. it keeps all the hidden states, as entries in a dictionary---------- THIS IS THE ACTUAL RNN STEP
