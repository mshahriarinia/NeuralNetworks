...
function a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  ...
  % optimization:
  % The optimizer that we're using is gradient descent with momentum, but
  % we'll need to find good values for the learning rate and the momentum multiplier.
...
function test_gradient(model, data, wd_coefficient)
  ...
  % We want to test the gradient not for every element of theta, because that's 
  % a lot of work. Instead, we test for only a few elements. If there's an error, 
  % this is probably enough to find that error.
  % We want to first test the hid_to_class gradient, because that's most 
  % likely to be correct (it's the easier one).
  ...
  for i = 1:100,
    ...
    for contribution_index = 1:8,
      temp = temp + loss(theta_to_model(base_theta + ...
          theta_step * contribution_distances(contribution_index)), data, wd_coefficient) * ...
...
% like the state of the hidden units. This is the forward pass, and you'll likely want to 
% copy it into d_loss_by_d_model, because these values are also very useful for that function.
...
function ret = loss(model, data, wd_coefficient)
  ...
  % The exponential in the lectures can lead to really big numbers, which are fine in mathematical 
  % equations, but can lead to all sorts of problems in Octave.
  % Octave isn't well prepared to deal with really large numbers, like the number 10 to the power 
  % 1000. Computations with such numbers get unstable, so we avoid them.
...
% This is the only function that you're expected to change. Right now, it just returns a 
% lot of zeros, which is obviously not the correct output. Your job is to change that.
...
function ret = classification_performance(model, data)
  ...
  [dump, choices] = max(class_input);
  
  % targets is integer: the target class, plus 1.
  [dump, targets] = max(data.targets); 
